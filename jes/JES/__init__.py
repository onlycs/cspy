...
def copyInto(smJESimg, lgJESimg, startX, startY):
    if smJESimg.PILimg.mode == "RGBA":
        lgJESimg.PILimg.paste(smJESimg.PILimg, (int(
            startX), int(startY)), smJESimg.PILimg)
    else:
        lgJESimg.PILimg.paste(smJESimg.PILimg, (int(startX), int(startY)))
...
def setRed(JESpix, red):
    ...
    JESpix.JESimg.PILimg.putpixel(
        (JESpix.x, JESpix.y), (red, color[1], color[2]))
...
def setBlue(JESpix, blue):
    ...
    color = JESpix.JESimg.PILimg.getpixel((JESpix.x, JESpix.y))
    JESpix.JESimg.PILimg.putpixel(
        (JESpix.x, JESpix.y), (color[0], color[1], blue))
...
def setGreen(JESpix, green):
    ...
    color = JESpix.JESimg.PILimg.getpixel((JESpix.x, JESpix.y))
    JESpix.JESimg.PILimg.putpixel(
        (JESpix.x, JESpix.y), (color[0], green, color[2]))
...
def addRect(JESimg, startX, startY, width, height, color=(0, 0, 0)):
    d = ImageDraw.Draw(JESimg.PILimg, mode="RGB")
    d.rectangle([int(startX), int(startY), int(startX+width),
                int(startY+height)], outline=color, fill=None)


def addRectFilled(JESimg, startX, startY, width, height, color=(0, 0, 0)):
    d = ImageDraw.Draw(JESimg.PILimg, mode="RGB")
    d.rectangle([int(startX), int(startY), int(startX+width),
                int(startY+height)], outline=color, fill=color)
...
def addOval(JESimg, startX, startY, width, height, color=(0, 0, 0)):
    d = ImageDraw.Draw(JESimg.PILimg, mode="RGB")
    d.ellipse([int(startX), int(startY), int(startX+width),
              int(startY+height)], outline=color, fill=None)
...
def addArc(JESimg, startX, startY, width, height, start, angle, color=(0, 0, 0)):
    d = ImageDraw.Draw(JESimg.PILimg, mode="RGB")
    d.arc([int(startX), int(startY), int(startX+width), int(startY+height)],
          int(360-start-angle), int(360-start), fill=color)


def addArcFilled(JESimg, startX, startY, width, height, start, angle, color=(0, 0, 0)):
    d = ImageDraw.Draw(JESimg.PILimg, mode="RGB")
    d.pieslice([int(startX), int(startY), int(startX+width), int(startY+height)],
               int(360-start-angle), int(360-start), outline=color, fill=color)
...
def writeMovieTo(images, filename):
    ...
    images[0].save(filename,
                   # loop=3)
                   save_all=True, append_images=images[1:], optimize=False, duration=40)
